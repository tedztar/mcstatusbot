...
async function noMonitoredServers(guildId, interaction, isStatusCommand) {
	...
	if (!monitoredServers.length) {
		interaction && sendMessage(interaction, `There are no monitored servers${isStatusCommand ? ', and no IP address was specified!' : '!'}`);
		return true;
	}
...
async function isServerUnspecified(server, guildId, interaction) {
	if ((await multipleMonitoredServers(guildId)) && !server) {
		interaction && (await sendMessage(interaction, 'There are multiple monitored servers, and no server was specified!'));
		return true;
	}
...
async function removingDefaultServer(server, guildId, interaction) {
	if ((await multipleMonitoredServers(guildId)) && (await isDefault(server, guildId))) {
		interaction && (await sendMessage(interaction, 'There are multiple monitored servers, and this server is the default server!'));
		return true;
	}
